...
const bool DEBUG = true; // MAKE FALSE BEFORE SUBMISSION!
...
public:
	...
	WayEntry_p(const unsigned &num_of_sets) :valid(false), dirty(false) ,tag(0),last_accessed_time(__UINT32_MAX__)
	{
	...
	friend ostream &operator<<(ostream &os, const WayEntry_p &entry)
		{
			...
			os << entry.valid <<" "  <<entry.last_accessed_time <<" "  <<entry.dirty << " 0x" << std::setfill('0') << std::setw(8) << std::hex << entry.tag << ' ' << std::dec;		
			return os;
		}
};
...
public:
	...
	//	  const unsigned &L2Cyc, const unsigned &WrAlloc) : MemCyc(MemCyc), Bsize(BSize), L1Size(L1Size), L2Size(L2Size), L1Assoc(L1Assoc), L2Assoc(L2Assoc),
	//														L1Cyc(L1Cyc), L2Cyc(L2Cyc), write_policy(WrAlloc), L1MissRate(0), L2MissRate(0),
	//														avgAccTime(0), L1(CacheLevel(BSize, L1Assoc, L1Cyc, L1Size)), L2(CacheLevel(BSize, L2Assoc, L2Cyc, L2Size))
		Cache(const unsigned &MemCyc, const unsigned &BSize, const unsigned &L1Size, const unsigned &L2Size,
		  const unsigned &L1Assoc, const unsigned &L2Assoc, const unsigned &L1Cyc,
		  const unsigned &L2Cyc, const unsigned &WrAlloc) : t_access(0), MemCyc(MemCyc), write_policy(static_cast<WritePolicy>(WrAlloc)), L1MissRate(0), L2MissRate(0),
															avgAccTime(0),
															L1(CacheLevel(L1Size, L1Assoc, L1Cyc,BSize,1 )),
															L2(CacheLevel(L2Size, L2Assoc, L2Cyc, BSize,2))
	{
		//L1 = new CacheLevel(BSize, L1Assoc, L1Cyc, L1Size);
		//L2 = new CacheLevel(BSize, L2Assoc, L2Cyc, L2Size);
	}
		...
		if(!requested_entry)
		{
			...
			if(!requested_entry)
			{
				...
				{					
					...
					// requested_entry = entry after fetching
					...
					if(l2_victim->valid)
					{
						...
						WayEntry entry_l1 =NULL;//= L1.blockExists();
						if(entry_l1)
						{
					...
					if(l1_victim->valid)
					{
						// Entry was valid there and dirty!!
						// Need to update L2 with new data
						// Should change acces time in L2 as well?
						//unsigned int pseudo_address = entry_l1.
	...
	friend ostream &operator<<(ostream &os, const Cache &cache)
		{
			os << "-----------------------------------------------------------------" << endl;
			os << "Cache L1" << endl <<endl;
			os << cache.L1<< endl <<endl;;
			os << "-----------------------------------------------------------------" << endl;
			os << "Cache L2" << endl <<endl;
			os << cache.L2 << endl;
			os << "-----------------------------------------------------------------" << endl;			
			return os;
		}
